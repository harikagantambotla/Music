...
window.addEventListener("load", ()=>{
    loadMusic(musicIndex);     //calling load function  once window loaded
    playingNow();
})
...
//upadte progress bar width according to music current time
...
progressArea.addEventListener("click", (e)=>{
    ...
    let clickedOffSetX = e.offsetX; //getting song total duration
...
repeatBtn.addEventListener("click",()=>{
    ...
    switch(getText){
        case "repeat": //if this icon is repeat then change it to repeat_one
           ...
           break;
        case "repeat_one": //if this icon is repeat then change it to shuffle
           repeatBtn.innerText = "shuffle";
           repeatBtn.setAttribute("title", "Playback shuffle");
           break;   
        case "shuffle": //if this icon is repeat then change it to repeat
...
mainAudio.addEventListener("ended", ()=>{
    ...
    let getText = repeatBtn.innerText; //getting innerText of icon
    //let's do different changes on different icon click using swich
    switch(getText){
        ...
        case "shuffle": //if icon is shuffle then change it to repeat
            ...
            }while(randIndex == randIndex); //this loop run until the next random number won't be  the same 
...
for (let i = 0; i < allMusic.length; i++){
    ...
    let liTag = ` <li li-index="${i + 1}">
                  ...
                  <span id="${allMusic[i].src}" class="audio-duration">4:45</span>
                <li>`;
    ...
    liAudioTag.addEventListener("loadeddata", ()=>{
        ...
        //adding t duration attribute which 0 if sec is less than 10
...
for (let j = 0; j < allLiTags.length; j++){
    let audioTag = allLiTags[j].querySelector(".audio-duration");
     ...
     //if there is an li tag which li tag which li-index is equal to musicIndex
